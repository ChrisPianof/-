...
## usePersisted: семантика валидатора

Хук `usePersisted(key, initial, validate)` в `BasePage.tsx` читает localStorage и проверяет результат через `validate`. **Гочча:** валидатор-как-схема пропускает значения, которые корректны по форме, но семантически не должны быть стартовым state'ом. Пример — пустой массив:

```ts
const isIdItemArray = (v) => Array.isArray(v) && v.every(it => ...);
// isIdItemArray([]) === true (every() на пустом = vacuous truth)
```

Если в localStorage `[]` (например после первой сборки с дефолтом `[]`), хук вернёт `[]`, и новый дефолт `[{id:0}]` НЕ применится — даже после bump'а ключа: HMR Vite не пересоздаёт `useState`, persisted `useEffect` пишет старое значение в новый ключ.

**Решение:** валидатор должен отражать СЕМАНТИКУ, не только схему. Для «обязательно хотя бы 1 элемент»:

```ts
(v): v is IdItem[] => isIdItemArray(v) && v.length > 0
```

Тогда `[]` отвергается → дефолт применяется чисто.
