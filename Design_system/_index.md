...
При получении ссылки на Figma-паттерн / компонент от юзера — workflow в `AlfaBank/Rules.md` → разделы «Workflow: Figma → Design_system → Код», «Workflow: Код → Figma», «Edge cases», «.fig — точка контакта». `.fig` файлы в Google Drive — точка контакта между нами; `.md` — точка истины для правил применения в коде.
...
Дизайн-система основана на Alfa Business Guidelines (паттерны `ЭтоБаза ::` в Figma). Все правила перенесены и живут локально в .md файлах Design_system — ссылки ниже только для traceability, не как источник истины.
...
Локальные .fig (Google Drive, синкается на диск):
Путь: ~/Library/CloudStorage/GoogleDrive-sweeptrip@gmail.com/My Drive/Claude/
Содержит .fig экспорты паттернов Альфа-банка. Пополняется. Используется для изучения нового паттерна или перепроверки старого. Открывать через `open <путь>` — запустит Figma desktop.
ВАЖНО: имя папки может меняться (была `Сlaude` с кириллической С, стала `Claude` латиницей). Перед обращением — проверять текущее имя через `ls "~/Library/CloudStorage/GoogleDrive-sweeptrip@gmail.com/My Drive/"`.

Текущее содержимое:
- ✅ Компонент __ TitleView.fig → связан с TitleView.md
...
При появлении новой команды от пользователя — дописывать строку сюда без переспроса.

Связанные паттерны из Guidelines:
...
- `ЭтоБаза :: Статусная модель` → Status.md (палитра, лексикон, алгоритм выбора цвета)

Нейминг конвенций Guidelines (Figma-side):
- Паттерны: `ЭтоБаза :: Название`
- Документация: `💊Documentations / Plates / Cell`
- Связанные: `💊Connected pattern`
- Локальные (только Figma, без реализации в коде): `❌ LocalComponents :: Figma Only`
